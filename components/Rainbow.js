...
const RainbowMaterial = shaderMaterial(
  ...
  /*glsl*/ ` varying vec2 vUv;
    ...
    vec3 iridescence (float angle, float thickness) {      
      ...
      return vec3((film * 20.0 + pow(f,40.0))) * tint;
    }
    ...
    void main() {
      const vec2 vstart = vec2(0.1, 0.5);
      // const vec2 vstart = vec2(0.1, -0.5);
      const vec2 vend = vec2(1, 0.5);
      // const vec2 vend = vec2(1, -2);
...
export const Rainbow = forwardRef(({ startRadius = 0, endRadius = 0.5, emissiveIntensity = 2.5, fade = 0.25, ...props }, fRef) => {
  ...
  return (
    <mesh ref={fRef} scale={[length, length, 1]} {...props}>
      <planeGeometry />
      <rainbowMaterial ref={material} key={RainbowMaterial.key} fade={fade} startRadius={startRadius} endRadius={endRadius} ratio={1.85} toneMapped={false} />
    </mesh>
  )
})
